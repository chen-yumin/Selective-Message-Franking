...
// 单次完整流程测试，返回四阶段统计
static void run_single_test(PP& pp, KeyPair& KPs, KeyPair& KPr, KeyPairj& KPj,
        ...
        int warmup_round = 100, int test_round = 500)
{
	...
	//int total_sum = *(uint32_t*)msg;
	//int total_sums = 0;
	//for(int i = 0; i < total_sum; i++) {
		//total_sums += *(uint32_t*)(msg+4+4*i);
	//}
	//printf("分片后的总大小: %d\n", total_sums);
    ...
    for (int w = 0; w < warmup_round; w++)
    {
        ...
        Report(aux, srm, sigma, msg, T_arr, sig, pp, KPs.pk, KPj.pk, KPr.sk);
...
int main()
{
    ...
    std::ofstream f1("ecdsa_data_size_curve.csv");
    ...
    f1.close();
    printf("模块1完成，数据保存至 data_size_curve.csv\n\n");

    // ========== 模块2：固定500KB，多梯度分块、过半上报，输出 block_grad_curve.csv ==========
    std::ofstream f2("ecdsa_block_grad_curve.csv");
    ...
    for (int blk : block_grad)
    {
		...
		EncodeResult* res = split_jpeg_file_encode(img_path, blk, 80);
    ...
    f2.close();
    //remove(fixed_txt);
    printf("模块2完成，数据保存至 ecdsa_block_grad_curve.csv\n\n");

    printf("全部测试完成！\n");
    printf("1. 尺寸-延迟曲线绘图数据：ecdsa_data_size_curve.csv\n");
    printf("2. 分块数量-延迟曲线绘图数据：ecdsa_block_grad_curve.csv\n");
    return 0;
}
